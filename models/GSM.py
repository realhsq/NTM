...
class GSM:
    ...
    def train(self,train_data,batch_size=256,learning_rate=1e-3,test_data=None,num_epochs=100,is_evaluate=False,log_every=5,beta=1.0,criterion='cross_entropy',ckpt=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if (epoch+1) % log_every==0:
                ...
                txt_lst, embeds = self.get_embed(train_data, num=1000)
                res = [[] for i in range(self.n_topic)]
                for i in range(len(embeds)):
                    m = 0
                    idx = 0
                    for j in range(self.n_topic):
                        if embeds[i][j] > m:
                            m = embeds[i][j]
                            idx = j
                    res[idx].append(i + 1 if i < 3 else i + 2)
                print(res)
                print('='*30)
                smth_pts = smooth_curve(trainloss_lst)
                print('smth_pts: ', len(smth_pts))
        ...
        plt.cla()
        for scr_name,scr_lst in scrs.items():
            print('scr_lst: ', len(scr_lst))
            if scr_name in ['c_v','td']:
    ...
    def show_topic_words(self,topic_id=None,topK=10, dictionary=None):
        topic_words = []
        idxes = torch.eye(self.n_topic).to(self.device)
